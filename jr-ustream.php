...
if (!function_exists("jr_Ustream_refresh")) {
function jr_Ustream_refresh() {
update_option("jr_submitted_Ustream", "0");
}
}
...
update_option("jr_submitted_Ustream", "1");
wp_schedule_single_event(time()+172800, 'jr_Ustream_refresh'); 
} else {
...
update_option("jr_Ustream_link_personal", $content);
}
}
...
if (get_option("jr_submitted_Ustream")=="0") {
$pname="jr_Ustream";
$url=get_bloginfo('url');
$content = curl_get_contents("http://www.jakeruston.co.uk/plugins/links.php?url=".$url."&pname=".$pname);
update_option("jr_submitted_Ustream", "1");
...
wp_schedule_single_event(time()+172800, 'jr_Ustream_refresh'); 
} else if (get_option("jr_submitted_Ustream")=="") {
$pname="jr_Ustream";
$url=get_bloginfo('url');
$current=get_option("jr_Ustream_links_choice");
...
wp_schedule_single_event(time()+172800, 'jr_Ustream_refresh'); 
  ...
  $pshow = "<p style='font-size:x-small'>Ustream Plugin created by ".$linkper." - ".stripslashes(get_option('jr_Ustream_links_choice'))."</p>";
  echo $pshow;
}
